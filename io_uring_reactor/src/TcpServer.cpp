...
TcpServer::TcpServer(EventLoop *loop, const InetAddress &listenaddr, const string &name, Option option)
    ...
    ip_port_(listenaddr.get_ip_port()), 
    name_(name), 
    ...
    started_(0)
{
    ...
    // loop_->poller_->add_accept(acceptor_->accept_socket_.get_fd(), (struct sockaddr *)client_addr, &client_len, 0);
    // LOG_INFO("new connection callback set\n");
    LOG_INFO("TcpServer initialized \n");    
}
TcpServer::~TcpServer()
{
...
{
    LOG_INFO("new connection comes \n");

    EventLoop *ioloop = loop_;  // 不使用多个loop
